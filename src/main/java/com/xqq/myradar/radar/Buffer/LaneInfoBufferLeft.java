...
@Component
public class LaneInfoBufferLeft {
    @Autowired
    ...
    private  static LaneInfoBufferLeft laneInfoBuffer=new LaneInfoBufferLeft();
    public LaneInfoBufferLeft(){};

    public static LaneInfoBufferLeft getS(){
    ...
    public  void select(String fiberName,String radarName){
        ...
        List<LaneInfo> radarlaneinfo_left = this.laneInfoMapper.selectAll(radarName);//这里把最新的微波雷达静态数据表写死了
        for (int i = 0; i < radarlaneinfo_left.size(); i++) {
            //如果它不在map中就新增
            if (!radarlaneinfo_left.get(i).getRadarId().equals("0")) {
                if (!laneInfomap.containsKey(radarlaneinfo_left.get(i).getRadarId())) {
                    List<LaneInfo> laneList = new LinkedList<>();
                    laneList.add(radarlaneinfo_left.get(i));
                    laneInfomap.put(radarlaneinfo_left.get(i).getRadarId(), laneList);
                }
                //如果它在map中，就在它后面加上
                else {
                    List<LaneInfo> laneListTEMP = laneInfomap.get(radarlaneinfo_left.get(i).getRadarId());
                    laneListTEMP.add(radarlaneinfo_left.get(i));
                }
            }
        }
